...
var Renderer = mini.Class.subclass({
  ...
  pushViewport: function(viewport) {
    ...
    this.context.scale(1 / viewport.extent.x, 1 / viewport.extent.y);
    // move current world camera point to the coordinate system's origin
    this.context.translate(-viewport.point.x, -viewport.point.y);

    this.singlePixelExtent = viewport
      .screenToWorldCoordinates(Vector2.One.copy())
      .sub(viewport.screenToWorldCoordinates(Vector2.Zero.copy()));
  ...
  drawPlus: function(point, size, color, opacity, lineWidth) {
    ...
    // draw a polyline
    this.context.moveTo(point.x - this.singlePixelExtent.x * size, point.y);
    this.context.lineTo(point.x + this.singlePixelExtent.x * size, point.y);
    this.context.moveTo(point.x, point.y - this.singlePixelExtent.y * size);
    this.context.lineTo(point.x, point.y + this.singlePixelExtent.y * size);
  ...
  drawTextWorld: function(text, worldPoint, color, opacity, baseline) {
    ...
    this.context.translate(worldPoint.x, worldPoint.y);
    this.context.scale(this.singlePixelExtent.x, this.singlePixelExtent.y);
  ...
  drawText: function(text, screenPoint, color, opacity, baseline) {
    ...
    this.context.fillText(text, screenPoint.x, screenPoint.y);
  },
  ...
  drawSystem: function(system) {
    // draw grid nodes
    if (system.drawGrid) this.drawGrid(system.grid);

    // draw obstacles
    if (system.doObstacles) {
      for (let obstacles of system.obstacles) {
  ...
  drawParticle: function(particle) {
    ...
    var dirLength = Math.max(
      this.singlePixelExtentLength,
      particle.gridVelocity.length()
    );

    this.drawLine(
      particle.position,
      particle.position.add(
        particle.gridVelocity.normalizedCopy().mulFloat(dirLength)
      ),
...
Renderer.springColorScale = d3.scale
  .linear()
  .domain([-3, 3])
  .range(["#ff0000", "#0000ff"]);
